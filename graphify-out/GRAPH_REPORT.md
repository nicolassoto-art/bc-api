# Graph Report - bc-api  (2026-08-01)

## Corpus Check
- 180 files · ~135,694 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1310 nodes · 2288 edges · 146 communities (102 shown, 44 thin omitted)
- Extraction: 96% EXTRACTED · 4% INFERRED · 0% AMBIGUOUS · INFERRED: 87 edges (avg confidence: 0.79)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1f55ba51`
...
- `main()` --indirect_call--> `Proyecto`  [INFERRED]
  scripts/backup_stock_diario.py → app/models/proyecto.py
...
Cohesion: 0.11
Nodes (30): _aplicar_excel(), _dominio_de(), _extract_body_text(), _extract_from_original(), _extraer_nombre_proyecto_del_excel(), _fetch_new(), _html_err(), _html_ok() (+22 more)

### Community 3 - "Gestion de inmobiliarias (catalogo)"
Cohesion: 0.08
Nodes (34): Base, Re-export models for Alembic autogenerate + convenience., Inmobiliaria, Catálogo maestro de inmobiliarias.  Antes vivía en localStorage del navegador de, Tickets de reporte de fallas (Fase 5).  Cualquier usuario autenticado de Herrami, Ticket, actualizar(), crear() (+26 more)
...
Cohesion: 0.06
Nodes (32): Workflow: Audit Plantas, Workflow: Batch import JB (encadenamiento dinámico, L-V 10-18 Chile), Workflow: Batch re-import (plantas + vacíos), Workflow: CI, Workflow: Import ALL pending JB (uno a la vez, hasta terminar), Workflow: Import JetBrokers project to bc-api, Workflow: List JetBrokers projects, Workflow: Monitor health bc-api + cadena import (+24 more)

### Community 7 - "Modulo JBImporter (importador core)"
Cohesion: 0.06
Nodes (28): JBImporter, Lee el valor de un input/select. Lo normaliza a string o None., 1D - 1B" -> (1, 1). bc-api deriva extra tipologia de la columna Modelo         c, Construye un .xlsx formato JB (INSTRUCCIONES/UNIDAD/ESTACIONAMIENTOS/         BO, Build payload completo y PUT., Workflow: Audit fotos+plantas vs JB, Workflow: Diag Click (detalle real), Workflow: Diag detail page (proyectos propios) (+20 more)
...
Cohesion: 0.29
Nodes (14): aplicar_patch(), clasificar(), descargar_y_preparar_imagen(), es_candidata(), escribir_summary(), health_check(), listar_imagenes(), listar_proyectos() (+6 more)
...
Cohesion: 0.20
Nodes (21): api_login(), fetch_catalog(), fetch_parking(), fetch_project_detail(), fetch_storage(), fetch_units(), _float(), get_token() (+13 more)
...
Cohesion: 0.08
Nodes (28): preview_operador_today(), HTML del informe de las 13:00 (avances de HOY de Cristofer) con datos REALES,, _build_html(), _build_operador_html(), _calidad_band(), _catalogo_vs_stock_html(), _disclaimer_html(), _editor_url() (+20 more)

### Community 17 - "Reporte de actividad del operador"
Cohesion: 0.08
Nodes (42): _age_hours(), _alertas_de_proyecto(), _antiguedad_color(), build_daily_report(), build_operador_today(), _calidad_score(), _catalogo_vs_stock(), _critico_key() (+34 more)
...
Cohesion: 0.06
Nodes (42): Documento, Proyecto + entidades hijas (unidades, imágenes, documentos).  El modelo refleja, actualizar(), DocumentoUpdate, DocumentoUrlIn, eliminar(), _ensure_project(), listar() (+34 more)
...
Cohesion: 0.12
Nodes (13): _acquire_scheduler_lock(), diag_email(), diag_usuario(), bc-api · backend privado para Herramientas BigCapital.  Uvicorn entry: `uvicorn, Dispara el informe diario manualmente (solo super_admin) y lo ENVÍA a los     de, Dispara y ENVÍA el informe de las 13:00 a operador_report_to (solo super_admin)., Estado de la fila bc-api de un usuario (solo super_admin). Para diagnosticar, Dispara el procesador de inbox manualmente (solo super_admin). Lee emails con (+5 more)
...
Cohesion: 0.16
Nodes (17): get_db(), SQLAlchemy engine + session factory., FastAPI dependency: yields a SQLAlchemy session per request., current_user(), Session, FastAPI dependencies for auth: extract user from Authorization header., Valida el token de servicio del Cloudflare Worker (catálogo público).      Compa, Acceso a Stock propio: super admin O usuario con permiso de stock.      El permi (+9 more)
...
Cohesion: 0.50
Nodes (3): Workflow: Diag Rosas 1444, main(), diag_rosas.py — Inspecciona modelos crudos de Rosas 1444 (5 mod / 1 planta disti
...
Cohesion: 0.06
Nodes (24): jb_importer.py — Importador JetBrokers → bc-api.  Módulo reusable. Diseñado para, Sentinel interno para saltar una sección de scrape_marketplace_workview     en m, _SkipSection, Exception, Workflow: Diag completo proyecto (qué falta), Workflow: Diag filtros unidades, Workflow: Diag investigate (Terrazzo + 0-uds + Abdón), Workflow: Diag vm scroll (+16 more)

### Community 63 - "Diagnostico completo de proyecto"
Cohesion: 0.50
Nodes (3): Workflow: Diag Sniff API (solo lectura), main(), diag_sniff_api.py — Captura el CONTRATO REAL de la API JB 7.43.1 (solo lectura).
...
Cohesion: 0.20
Nodes (9): Eventos anómalos — registro automático, 🚨 Fallos de workflow (27 en total, mostrando últimos 27), ℹ Batches fuera de ventana L-V 10-18 (60 en total, mostrando últimos 30), ℹ Errores leyendo GH CLI (1 en total, mostrando últimos 1), ℹ Inmobiliaria sin asignar (32 en total, mostrando últimos 30), ℹ Nombre stub no actualizado (2 en total, mostrando últimos 2), ℹ Workflows cancelados (120 en total, mostrando últimos 30), ⚠ Proyectos con modelos pero sin unidades (115 en total, mostrando últimos 30) (+1 more)
...
Cohesion: 0.22
Nodes (16): Workflow: Patch stock (no-destructivo), asset_m2(), asset_num(), asset_uf(), _fnum(), main(), patch_stock.py — Refresca estac/bodegas/packs de un proyecto SIN wipe. Trae el m, _card() (+8 more)
...
- **Why does `JBImporter` connect `Modulo JBImporter (importador core)` to `diag_api_explore.py`, `Rutas de unidades (deptos)`, `import_marketplace_workview.py`, `diag_bod_raw.py`, `Reporte de importacion y utilidades scraping`, `diag_chips_full.py`, `diag_vm_cotizar.py`, `diag_vm_typesel.py`, `diag_workview.py`, `Eventos anomalos y workflows batch`, `Descarga y gestion de assets JB`, `Modelos de datos (Proyecto, Ticket)`, `Mapeo de campos JB hacia bc-api`, `Diagnostico de scraping en vivo`, `Test de paridad DOM (JB vs BC)`, `Rutas de imagenes del proyecto`, `Verificacion visual con IA (AI Vision)`, `Calculo de metricas del informe diario`, `Test de paridad UI campo a campo`, `Motor de alertas de proyecto (criticos)`, `Patch no-destructivo de stock`, `Scraping de superficies individuales`, `CLI de importacion JetBrokers`, `Listado de proyectos JetBrokers`, `Auditoria de fotos y plantas vs JB`, `Diagnostico de stock total JB`, `Diagnostico de autenticacion (401)`, `Caza del endpoint de detalle JB`, `Diagnostico de stock en marketplace`, `Confirmacion del pipeline API-first JB`, `Diagnostico de estac/bodegas extra`, `Ranking de pendientes por stock`, `Diagnostico de assets (estac/bodegas)`, `Diagnostico de carga de detalle (click)`, `Diagnostico completo de proyecto`, `Scraping de etiquetas JB`, `Diagnostico de proyectos Euro`, `Diagnostico de headers (fix 401)`, `Diagnostico de tipos en marketplace`, `Diagnostico del paginador de unidades`, `Diagnostico de tab Stock del workview`, `Diagnostico de selector Tipo en Stock`, `diag_csv_batch.py`?**
  _High betweenness centrality (0.317) - this node is a cross-community bridge._
- **Why does `Dependencias Python (requirements.txt)` connect `Eventos anomalos y workflows batch` to `Reporte de actividad del operador`?**
  _High betweenness centrality (0.057) - this node is a cross-community bridge._
- **Why does `Usuario` connect `Rutas admin: informes e inmobiliarias` to `Rutas de unidades (deptos)`, `Gestion de inmobiliarias (catalogo)`, `Infraestructura DB y autenticacion`, `Permisos de acceso a Stock/Worker`, `Autenticacion y esquemas de sesion`, `Vista previa del informe diario`, `Ruta de importacion batch (API)`, `Generacion HTML del informe diario`, `Rutas de documentos del proyecto`, `CI/CD, despliegue y seed inicial`?**
